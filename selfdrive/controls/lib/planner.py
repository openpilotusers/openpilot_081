...
class Planner():
  ...
  def update(self, sm, pm, CP, VM, PP):
    ...
    if enabled and not self.first_loop and not sm['carState'].brakePressed and not sm['carState'].gasPressed:
      ...
      tspeed = 0
      if (v_ego*3.6) <= 50:
        tspeed = v_ego*3.6*2.5
      elif (v_ego*3.6) <= 70:
        tspeed = v_ego*3.6*3.5
      elif (v_ego*3.6) > 70:
        tspeed = v_ego*3.6*4.5

      if sm['liveMapData'].speedLimitValid and 0 < sm['liveMapData'].speedLimitAheadDistance < tspeed:
        self.v_speedlimit_ahead = sm['liveMapData'].speedLimitAhead
        self.v_speedlimit_ahead_timer = 0
      else:
        self.v_speedlimit_ahead_timer += 1
        if self.v_speedlimit_ahead_timer > (v_ego*3.6*2):
    ...
    elif self.target_speed_map > 29 and self.target_speed_map_dist >= 6*v_ego*CV.MS_TO_KPH and self.target_speed_map_block:
      plan_send.plan.targetSpeedCamera = self.target_speed_map
    else:
